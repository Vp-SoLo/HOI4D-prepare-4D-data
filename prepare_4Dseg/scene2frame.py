...
def scene2frame(filelist, image_path, depth_path, mask_path, output_label_path, output_background_path, extrinsics, label, ckt, max_instance_label):
    ...
    for filename in filelist:
        ...
        # id = filename.split('.')
        #print(id)
        color_raw = o3d.io.read_image(os.path.join(image_path,id.zfill(5)+'.jpg'))
        depth_foregrounds, depth_background = convert(id, os.path.join(depth_path,id.zfill(5)+'.png'), os.path.join(mask_path, id.zfill(5)+'.png'), output_background_path)
...
def main(ex_path, image_path, depth_path, mask_depth, output_label_path, output_background_path, label3d_path):
    ...
    try:
        ...
        data_np = load_pcd_ascii(label3d_path)
    label = data_np[:,-2:]
