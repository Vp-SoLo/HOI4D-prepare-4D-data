...
def solve1(data_dir, data_2Dseg_dir, output_dir):
    ...
    mask_path = os.path.join(data_2Dseg_dir, "2Dseg", "shift_mask")
    ...
    label3d_path = os.path.join(data_dir, "3Dseg", "label.pcd")

    # extract_frames_from_align_rgb(image_path, "image.mp4", False)
    # extract_frames_from_align_depth(depth_path, "depth_video.avi", False)
...
def prepare_4Dseg_data(data_dir, output_dir, datalist_path):
    ...
    for video_name in datalist:
        ...
        #         cnt += 1
        # assert(cnt == 1)
        solve1(os.path.join(data_dir, video_name), os.path.join(data_dir, video_name), os.path.join(output_dir, video_name))
    print("------finish step 1!------")
...
def main():
    ...
    parser.add_argument('--data_dir', type=str, default="datasets/HOI4D/")
    parser.add_argument('--output_dir', type=str, default="./reconstructed")
    parser.add_argument('--train_datalist_path', type=str, default="./very_few_samples.txt")

    args = parser.parse_args()

    print("start preparing pointed set")
